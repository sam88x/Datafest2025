...
crime_data$state_name <- tolower(crime_data$state)

map_data <- left_join(states_map, crime_data, by = c("region" = "state_name"))
...
p <- ggplot() +
  ...
  scale_fill_gradient(name = "Percent Change in Violent Crime Rate (2019 to 2023)",
                      low = "dodgerblue", high = "darkred", 
                      labels = scales::percent_format(scale = 1)) +
  labs(title = "Percent Change in Violent Crime Rate (2019 - 2023) and Commercial Leasing Change",
  ...
  geom_segment(data = market_locations,
               aes(x = long, y = lat,
                   xend = long,
  ...
  geom_text(data = market_locations,
